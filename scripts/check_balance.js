...
assert.strictEqual(CONFIG.challenge.rulesVersion, 95, "challenge rules should bump when endless spawn pressure changes");
...
assert(CONFIG.endlessDifficulties.hell.playerHpMult > 1, "hell should keep its HP buffer");
assert.strictEqual(CONFIG.endlessDifficulties.hell.playerDmgMult, 3, "hell endless should start at 3x player damage");
...
assert.strictEqual(CONFIG.endless.dynamicHp.enemyDamageReduction, 0.05, "dynamic hell enemies should have 5% DR");
...
assert.strictEqual(CONFIG.chips.laserFocus.laserDamageBonus, 3, "laserFocus chip should provide visible laser growth");
assert.strictEqual(CONFIG.chips.homingSwarm.damageBonus, 3, "homingSwarm chip should provide visible homing growth");
assert.strictEqual(CONFIG.chips.missileBarrage.damageBonus, 3, "missileBarrage chip should provide visible missile growth");
...
assert(game.endlessEnemyHpMult() >= CONFIG.endless.enemyHpBaseMult * 0.5, "endless enemies should start with the halved base HP multiplier");
...
between(hellFloorAt40, 1, 6, "40s hell enemy floor should avoid a sudden HP wall");
between(normalFloorAt40, 1, 5, "40s normal enemy floor should avoid a sudden HP wall");
game._endlessDiffKey = "hell";
game._endlessT = CONFIG.endless.enemyHpBoostTime;
assert(Math.abs(game.endlessEnemyHpMult() - CONFIG.endlessDifficulties.hell.enemyHpBoostMult * 0.5) < 1e-9, "endless enemies should reach the halved hell HP boost at 60s");
assert.strictEqual(game.endlessEnemyHpFloor(), Math.round(CONFIG.endless.enemyHpFloor * CONFIG.endlessDifficulties.hell.enemyHpMult * 0.5), "enemy HP floor should reach the halved scaled opening floor at the HP boost time");
between(game.endlessEnemyHpFloor() / focusedDps.fourPick, 0.25, 0.8, "60s enemy floor TTK");
game._endlessT = CONFIG.endless.enemyHpBoostTime + CONFIG.endless.enemyHpDoubleInterval;
assert(Math.abs(game.endlessEnemyHpMult() - CONFIG.endlessDifficulties.hell.enemyHpBoostMult) < 1e-9, "endless enemies should double from the 60s HP boost after the global halving");
game._endlessT = CONFIG.endless.enemyHpBoostTime + CONFIG.endless.enemyHpDoubleInterval * 2;
assert(game.endlessEnemyHpMult() > CONFIG.endlessDifficulties.hell.enemyHpBoostMult * 2, "post-5m endless enemies should still exceed the old doubled curve after the global halving");
game._endlessT = CONFIG.endless.enemyHpFloorTargetTime;
assert.strictEqual(game.endlessEnemyHpFloor(), Math.round(CONFIG.endless.enemyHpFloorTarget * CONFIG.endlessDifficulties.hell.enemyHpMult * 0.5), "200s enemy floor should stay below the old wall");
between(game.endlessEnemyHpFloor() / focusedDps.strongRoute, 1.4, 2.3, "200s enemy floor TTK for strong focused fire");
game._endlessT = 300;
between(game.endlessEnemyHpFloor(), 2200, CONFIG.endless.enemyHpFloorMax, "300s enemy floor");
between(game.endlessEnemyHpFloor() / focusedDps.nearCap, 0.8, 1.2, "300s enemy floor TTK for near-cap focused fire");
...
assert.strictEqual(game.endlessSpawnWaveCount(9999), Math.max(1, Math.round(cappedSpawnCount * 0.5)), "boss presence should halve endless spawn waves");
game.boss = null;
...
game.endless = false; game.player = { power: 1, wings: 0 }; game._rng = () => 0.2;
assert.strictEqual(game.chooseDrop(), "power", "power drops should remain common before max power");
game.player = { power: CONFIG.player.maxPower, wings: 0 }; game._rng = () => 0.2;
assert.strictEqual(game.chooseDrop(), "heal", "max power should heavily reduce further power drops");
game.player = { power: CONFIG.player.maxPower, wings: CONFIG.wingMax - 1 }; game._rng = () => 0.8;
...
assert.strictEqual(toughEnemy.hp, 52.5, "dynamic hell enemy DR should reduce incoming damage by 5%");
...
approx(game.player.powerDamage, CONFIG.overflow.powerDamageStep, 1e-9, "overflow power should add reduced power damage");
approx(game.mainBulletDamage(null, "main"), mainDamageBeforePower + CONFIG.overflow.powerDamageStep, 1e-9, "overflow power should slightly increase main gun damage");
...
approx(new PlayerLaser(100, 100, 0).damage, laserDamageBeforePower + CONFIG.overflow.powerDamageStep, 1e-9, "overflow power should slightly increase laser damage");
...
approx(game.player.wingDamage, CONFIG.overflow.wingDamageStep, 1e-9, "overflow wing should add reduced wingman damage");
...
approx(game.mainBulletDamage(null, "wing"), wingDamageBeforeWing + CONFIG.overflow.wingDamageStep, 1e-9, "overflow wing should slightly increase wingman bullets");
game.ship = CONFIG.ships.defender; game.diff = CONFIG.difficulties.normal; game.bonuses = {}; game.chips = {}; game.enemies = []; game.floats = []; game.particles = []; game.shockwaves = []; game.player = new Player();
game.useSpecialShield();
assert.strictEqual(game.player.shieldHits, 2, "defender special shield should start with two blocks");
game.player.takeDamage(1);
assert.strictEqual(game.player.shieldHits, 1, "defender special shield should spend one block per hit");
assert(game.player.shieldHp > 0, "defender special shield should survive the first small hit");
game.player.invulnTimer = 0;
game.player.takeDamage(1);
assert.strictEqual(game.player.shieldHp, 0, "defender special shield should expire after two blocked hits");
assert.strictEqual(game.player.shieldHits, 0, "defender special shield hit counter should clear when spent");
game.player.grantShield(10, 4);
assert.strictEqual(game.player.shieldHits, 0, "normal shields should not inherit defender hit limits");
